<template>
    <div id="videoContainer"
         class="absolute top-0 left-0 w-full h-full flex justify-center items-center text-red-600 bg-black">
    ...
    <div class="absolute top-0 left-0 w-full h-full flex justify-center items-end text-red-600">
        <button @click="takePicture"
                class="w-12 h-12 rounded-full border-gray-400 border-4 shadow-2xl mb-5 bg-white"></button>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async takePicture() {
            ...
            this.$store.commit('addForm', [this.formsCant, src, true, this, true])
            this.video = null
            this.canvas = null
